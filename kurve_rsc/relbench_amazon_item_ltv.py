...
def _build_feature_frames(
    ...
    frame_store = RelBenchFrameStore("rel-amazon-item-ltv-features", persist_each_frame=True)
    ...
    def build_frame(frame_con: duckdb.DuckDBPyConnection, task_timestamp: pd.Timestamp) -> pd.DataFrame:
        ...
        apply_feature_family_policy([customer_node, product_node, review_node])
...
def run_rel_amazon_item_ltv(
    ...
    task = get_relbench_task("rel-amazon", "item-ltv", download=True)
    ...
    try:
        ...
        for split_name in ["train", "val", "test"]:
            ...
            split_store = RelBenchFrameStore(
                f"rel-amazon-item-ltv-{split_name}", persist_each_frame=True
            )
            for features in feature_store.iter_batches():
                ...
                split_store.append(frame)
            feature_store.close()
            split_stores[split_name] = split_store
    finally:
        con.close()
    ...
    feature_columns = [
        column
        for column in train_sample.select_dtypes(include=["number"]).columns
        if column != target
    ...
    config = {
        "name": "amazon_item_ltv_incremental",
        "iterations": 700,
        "depth": 8,
        "learning_rate": 0.05,
        "l2_leaf_reg": 3.0,
