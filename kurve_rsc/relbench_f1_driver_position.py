#!/usr/bin/env python
"""RelBench rel-f1: driver position example aligned to the official task definition."""
...
def _task_table_for_split(split_name: str):
    return get_relbench_split_task_table(
        DATASET_NAME, "driver-position", split_name, download=True
    )


def run_rel_f1_driver_position(
    ...
    try:
        ...
        for split_name in ["train", "val", "test"]:
            ...
            def build_frame(frame_con, cut_date):
                ...
                graph = GraphReduce(
                    name=f"rel_f1_driver_position_{pd.Timestamp(cut_date).date()}",
                ...
                graph.add_entity_edge(
                    driver_node,
                    result_node,
                    parent_key=driver_id_col,
                    relation_key=result_driver_col,
                    reduce=True,
                )
                graph.add_entity_edge(
                    driver_node,
                    standing_node,
                    parent_key=driver_id_col,
                    relation_key=standing_driver_col,
                    reduce=True,
                )
                graph.add_entity_edge(
                    result_node,
                    race_node,
                    parent_key=result_race_col,
                    relation_key=race_id_col,
                    reduce=False,
                )
                graph.add_entity_edge(
                    race_node,
                    circuit_node,
                    parent_key=race_circuit_col,
                    relation_key=circuit_id_col,
                    reduce=False,
                )
                graph.add_entity_edge(
                    result_node,
                    constructor_node,
                    parent_key=result_constructor_col,
                    relation_key=constructor_id_col,
                    reduce=False,
                )
...
def main() -> None:
    use_feature_manifest = feature_manifest_enabled("rel-f1/driver-position")
    df_train, df_val, df_test, val_metrics, test_metrics, n_features, materialized, target = run_rel_f1_driver_position(
