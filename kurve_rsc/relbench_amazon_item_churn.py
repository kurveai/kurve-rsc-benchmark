...
def run_rel_amazon_item_churn(
    ...
    official_task = get_relbench_task("rel-amazon", "item-churn", download=True)
    ...
    model, val_auc = fit_incremental_classifier(
        ...
        config={
            "name": "amazon_item_churn_incremental",
            "iterations": 2000,
            "depth": 6,
            "learning_rate": 0.02,
            "l2_leaf_reg": 10.0,
...
def main() -> None:
    ...
    print("train_cut_dates:", df_train.column_nunique("timestamp"), flush=True)
